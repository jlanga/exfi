...
Coordinate = namedtuple('Coordinate', ['seqid', 'start', 'end'])

# class Coordinate(Tuple[str, int, int]):
#     """Store coordinates as tuples of the shape str, int, int"""
#     def __init__(cls, seqid, start, end):
#         self = (seqid, start, end)
#
#     def __new__(cls, seqid, start, end):
#         """I don't know what the fuck i am doing:
#         http://jfine-python-classes.readthedocs.io/en/latest/subclass-tuple.html
#         """
#         return tuple.__new__(cls, (seqid, start, end))
    # seqid = None
    # start = None
    # end = None
    #
...
class SpliceGraph(nx.DiGraph):
    ...
    #         nx.set_node_attributes(G=self.splice_graph, name="coordinates",
    #             values=node2coordinates)
    #     else:
...
class Path2Nodes(Dict[str, Tuple[str, ...]]):
