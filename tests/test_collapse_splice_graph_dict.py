...
class TestComputeOld2New(TestCase):
    ...
    def test_empty(self):
        """exfi.collapse_splice_graph._compute_old2: empty case"""
        actual = _compute_old2new(SEQ2NODE_EMPTY)
        expected = OLD2NEW_EMPTY
        self.assertEqual(actual, expected)

    def test_simple(self):
        """exfi.collapse_splice_graph._compute_old2new: simple case"""
        actual = _compute_old2new(SEQ2NODE_SIMPLE)
        expected = OLD2NEW_SIMPLE
        self.assertEqual(actual, expected)

    def test_complex(self):
        """exfi.collapse_splice_graph._compute_old2new: complex case"""
